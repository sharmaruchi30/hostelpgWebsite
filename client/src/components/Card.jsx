...
import { setUserBookmarks } from "../state";
const Card = ({ cardType, houseInfo }) => {
  ...
  const bookmarkRoute = async (h_id, u_id) => {
    const url = `http://localhost:3001/house/${h_id}/${u_id}/bookmark`;
    try {
      fetch(url, {
        method: "PATCH",
        headers: {
        ...
        .then((json) => {
          ...
          console.log(json["newbookmark"]);
          dispatch(
            setUserBookmarks({
              bookmarks: json["newbookmark"],
            })
          );
          // console.log(bookmarks);
          window.location.reload(false);
      ...
      <div className="lastrow">
        ...
        {cardType === "settings" ? (
          ...
          >
            ...
            {/* <button
              class="carddeleteBtn cardeditBtn"
              onClick={() => navigate("/updateCard", { state: { houseInfo } })}
            >
              <svg
                // viewBox="0 0 448 512"
          ...
          <button
            class="bookmarkBtn"
            onClick={() => bookmarkRoute(houseInfo._id, userId)}
          >
            ...
            <p class="text">
              {bookmarks.includes(houseInfo._id) ? "remove" : "save"}
              {/* {bookmarks} */}
            </p>
          </button>
        ) : null}
